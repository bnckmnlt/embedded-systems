...
export const POST = async (req: Request, res: Response) => {
  try {
    ...
    if (recentRecord.length === 0) {
      await db.insert(distance).values({
        maxDistance: distance,
        minDistance: distance,
      });

      return NextResponse.json({ message: "Ultrasonic reading values were inserted successfully"});
    }
    else {
      ...
      await db.update(distance).set({
        maxDistance,
        minDistance,
      })
      .where(eq(distance.id, record.id));
